...
import { useXPToasts } from './XPToastNotification';
...
export default function DailyCheckin({ userId, onComplete }: DailyCheckinProps) {
  ...
  const { actions: xpActions } = useXP(userId);
  const { showToast } = useXPToasts();
  const { incrementStat, updateStat, updateSpecialFlag } = useBadges(userId);
  const { trackAction } = useChallenges(userId);
  ...
  const handleSubmit = () => {
    ...
    // 🎮 GAMIFICAÇÃO: Ganhar XP por check-in
    const xpGained = xpActions.dailyCheckin();
    if (xpGained) {
      showToast('Check-in Diário', xpGained);
    }

    // 🏆 BADGES: Atualizar estatísticas
    incrementStat('sessionsCompleted');
    
    // Atualizar streak atual
    const currentStreak = parseInt(localStorage.getItem(`claramente_streak_${userId}`) || '0');
    updateStat('currentStreak', currentStreak);

    // Verificar badges de horário especial
    const hour = new Date().getHours();
    if (hour >= 23 || hour < 6) {
      updateSpecialFlag('night_session', true);
    }
    if (hour >= 5 && hour < 7) {
      updateSpecialFlag('early_session', true);
    }

    // 🎯 DESAFIOS: Rastrear check-in
    trackAction('checkin');
