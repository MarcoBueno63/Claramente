...
function getTherapeuticResponse(count: number, userMessage: string, history: any[]) {
...
**Vamos fazer um exercício:** se um bom amigo seu tivesse exatamente os mesmos pensamentos, o que você diria para ele?`,
    ...
    case 4:
      return {
        message: `Interessante como conseguimos ter mais gentileza com outros, né? Que tal pensarmos em como aplicar isso conosco mesmos?
...
function getInitialConcernResponse(concern: string) {
  const responses = {
  ...
  return responses[concern as keyof typeof responses];
}

function getConcernValidation(concern: string, text: string) {
  const validations = {
  ...
  return validations[concern as keyof typeof validations] || validations.geral;
}

function getThoughtAnalysis(concern: string) {
  const analyses = {
  ...
  return analyses[concern as keyof typeof analyses] || analyses.geral;
}

function getPracticalTechnique(concern: string) {
  const techniques = {
  ...
  return techniques[concern as keyof typeof techniques] || techniques.geral;
}

function getSpecificTechnique(concern: string) {
  ...
  return techniqueNames[concern as keyof typeof techniqueNames] || techniqueNames.geral;
}

function getProgressValidation(concern: string) {
  return 'Você está demonstrando muita coragem ao explorar esses padrões e se abrir para novas perspectivas.';
}

function getIntegrationMessage(concern: string) {
  return 'O insight que você teve hoje é valioso. Pequenas mudanças de perspectiva podem ter grandes impactos.';
}

function getFinalReflection(concern: string) {
