...
import EmotionalKeyboard from './EmotionalKeyboard';
import XPBar from './XPBar';
import BadgeGallery from './BadgeGallery';
import WeeklyChallenges from './WeeklyChallenges';
import PersonalLeaderboard from './PersonalLeaderboard';
import XPToastNotification, { useXPToasts } from './XPToastNotification';
...
export default function ChatWindow() {
  ...
  const { toasts, showToast, dismissToast } = useXPToasts();
  ...
  // Estado para Daily Check-in e Onboarding
  ...
  const sendMessage = useCallback(async () => {
    ...
    try {
      ...
      if (response.ok && data.success) {
        ...
        // Resetar avatar antes de nova fala (garantir animação)
        setCurrentSpeechText('');
        
        // Pequeno delay para garantir reset completo
        setTimeout(() => {
          // Ativar avatar para falar a resposta
          console.log('🎤 Ativando avatar com texto:', data.response?.substring(0, 50));
  ...
  const detectSymptomsAndSuggestAssessment = useCallback((userMessage: string, assistantResponse: string) => {
    ...
    if (suggestedScalesForSymptoms.length > 0 && messages.length >= 10) {
      ...
      // Sugerir avaliação imediatamente
  ...
  const suggestTherapeuticExercises = useCallback((userMessage: string, assistantResponse: string) => {
    ...
    if (relevantExercises.length > 0 && messages.length >= 8) {
      ...
      if (exercise && indicator) {
        // Sugestão imediata para melhor performance
  ...
  return (
    <>
      {/* Onboarding Flow - Apenas para novos usuários */}
      {showOnboarding && (
        <OnboardingFlow onComplete={() => setShowOnboarding(false)} />
      )}
      
      {/* Daily Check-in - 1x por dia */}
      {showDailyCheckin && userAuth.user && (
        <DailyCheckin
          userId={userAuth.user.id}
          onComplete={(mood, note) => {
            console.log('Check-in concluído:', { mood, note });
      ...
      <div className="flex flex-col h-full bg-gradient-to-b from-blue-50 via-purple-50 to-pink-50 rounded-xl shadow-xl border-2 border-blue-200">
      {/* XP Bar */}
      {userAuth.user && (
        <div className="px-4 pt-4">
          <XPBar userId={userAuth.user.id} variant="compact" />
        </div>
      )}
      
      {/* Header */}
      <div className="flex items-center justify-between p-4 bg-white/90 backdrop-blur-sm border-b-2 border-blue-200 rounded-t-xl shadow-sm">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="font-bold text-gray-900 text-xl">Clara - Sua Terapeuta Virtual</h1>
            <p className="text-sm font-medium text-purple-700">Ambiente seguro e acolhedor para seu bem-estar</p>
          </div>
        </div>
        <div className="flex items-center gap-3">
          {/* Botão de Badges */}
          {userAuth.user && (
            <button
              ...
              className="px-3 py-2 bg-amber-100 text-amber-800 text-sm rounded-lg hover:bg-amber-200 transition-colors border-2 border-amber-300 font-semibold"
          ...
          {userAuth.user && (
            <button
              ...
              className="px-3 py-2 bg-teal-100 text-teal-800 text-sm rounded-lg hover:bg-teal-200 transition-colors border-2 border-teal-300 font-semibold"
              title="Ver desafios"
            >
              🎯 Desafios
            </button>
          ...
          {/* Sprint 5 Feature 4: Botão de Leaderboard/Progresso */}
          {userAuth.user && (
            <button
              onClick={() => setShowLeaderboard(true)}
              className="px-3 py-2 bg-purple-100 text-purple-800 text-sm rounded-lg hover:bg-purple-200 transition-colors border-2 border-purple-300 font-semibold"
              title="Ver progresso pessoal"
            >
              📊 Progresso
            </button>
          )}
          ...
          <a
            href="/report"
            className="px-3 py-2 bg-indigo-100 text-indigo-800 text-sm rounded-lg hover:bg-indigo-200 transition-colors border-2 border-indigo-300 font-semibold"
          ...
          {!assessmentCompleted && sessionActive && (
            <button
              ...
              className="px-3 py-2 bg-blue-100 text-blue-800 text-sm rounded-lg hover:bg-blue-200 transition-colors border-2 border-blue-300 font-semibold"
          ...
          <div className="text-right">
            <span className={`text-sm font-semibold ${sessionActive ? 'text-emerald-700' : 'text-rose-700'}`}>
              {sessionActive ? '💚 Sessão Ativa' : '✨ Sessão Finalizada'}
            </span>
            <p className="text-xs font-medium text-gray-700">
              {sessionActive ? `Terapia TCC/DBT/ACT | ${formatTime(timeRemaining)} restantes` : 'Sessão completa'}
            </p>
          </div>
          {timeRemaining <= 5 * 60 && sessionActive && (
            <div className="bg-amber-100 border-2 border-amber-400 rounded-lg px-3 py-2">
              <div className="flex items-center text-amber-900">
                <span className="text-sm font-semibold">⏰ Finalizando em breve</span>
              </div>
            </div>
          )}
        </div>
      ...
      {userAuth.user && !userAuth.loading && (
        <div className="px-6 py-3 bg-gradient-to-r from-purple-100 via-pink-100 to-blue-100 border-b-2 border-purple-200">
      ...
      {/* Chat Area */}
      <div className="flex-1 overflow-y-auto p-4 md:p-6 space-y-4 bg-gradient-to-b from-blue-50/50 via-purple-50/30 to-pink-50/20">
        {/* Avatar Clara - Fixo no topo */}
        {showAvatar && (
          <div className="sticky top-0 z-10 bg-gradient-to-r from-purple-100 via-pink-100 to-blue-100 rounded-2xl shadow-xl p-4 mb-6 border-2 border-purple-300">
            <div className="flex items-start gap-4">
              <div className="w-32 h-32 md:w-40 md:h-40 flex-shrink-0">
                <AvatarWithLipSync 
                  ...
                  onSpeakingComplete={() => {
                    console.log('🔊 Avatar terminou de falar');
                  }}
                />
              </div>
              <div className="flex-1">
                <div className="flex items-center justify-between mb-2">
                  <h3 className="text-lg font-bold text-gray-900">Clara - Sua Terapeuta Virtual 💜</h3>
                  <button
                    onClick={() => setShowAvatar(!showAvatar)}
                    className="text-gray-700 hover:text-gray-900 text-sm px-3 py-1 rounded-lg hover:bg-white/70 transition-colors font-medium"
                ...
                <p className="text-sm font-semibold text-gray-800 mb-3">
                  Assistente de Terapia Cognitivo-Comportamental com técnicas baseadas em evidência
                </p>
                <div className="flex flex-wrap gap-2 text-xs">
                  <span className="bg-blue-200 text-blue-900 px-3 py-1.5 rounded-lg font-bold border-2 border-blue-300">🧠 TCC</span>
                  <span className="bg-purple-200 text-purple-900 px-3 py-1.5 rounded-lg font-bold border-2 border-purple-300">🎯 DBT</span>
                  <span className="bg-green-200 text-green-900 px-3 py-1.5 rounded-lg font-bold border-2 border-green-300">🌱 ACT</span>
                </div>
              </div>
            </div>
          </div>
      ...
      {/* Input Area */}
      <div className="p-4 bg-gradient-to-r from-purple-50 to-pink-50 border-t-2 border-purple-200 rounded-b-xl">
      ...
      {/* Sprint 4 Feature - Emotional Keyboard */}
      <EmotionalKeyboard onSelectPhrase={(phrase) => setInputMessage(phrase)} />
      
      {/* Sprint 5 Feature 2 - Badge Gallery Modal */}
      {showBadgeGallery && userAuth.user && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white dark:bg-gray-900 rounded-2xl w-full max-w-6xl max-h-[90vh] overflow-auto shadow-2xl">
            ...
            <div className="p-6">
              <BadgeGallery userId={userAuth.user.id} variant="full" />
            </div>
          </div>
        </div>
      )}
      
      {/* Sprint 5 Feature 3 - Weekly Challenges Modal */}
      {showChallenges && userAuth.user && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white dark:bg-gray-900 rounded-2xl w-full max-w-6xl max-h-[90vh] overflow-auto shadow-2xl">
            <div className="sticky top-0 bg-white dark:bg-gray-900 border-b border-gray-200 dark:border-gray-700 p-4 flex items-center justify-between">
              <h2 className="text-2xl font-bold text-gray-800 dark:text-white">
            ...
            <div className="p-6">
              <WeeklyChallenges userId={userAuth.user.id} variant="full" />
            </div>
          </div>
        </div>
      )}
      
      {/* Sprint 5 Feature 4 - Personal Leaderboard Modal */}
      {showLeaderboard && userAuth.user && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white dark:bg-gray-900 rounded-2xl w-full max-w-6xl max-h-[90vh] overflow-auto shadow-2xl">
            <div className="sticky top-0 bg-white dark:bg-gray-900 border-b border-gray-200 dark:border-gray-700 p-4 flex items-center justify-between">
              <h2 className="text-2xl font-bold text-gray-800 dark:text-white">
            ...
            <div className="p-6">
              <PersonalLeaderboard userId={userAuth.user.id} variant="full" />
            </div>
          </div>
        </div>
      )}
