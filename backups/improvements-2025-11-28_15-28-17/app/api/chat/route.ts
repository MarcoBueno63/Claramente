...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    // Preparar contexto para OpenAI - usar APENAS histórico existente + mensagem atual
    ...
    // SEMPRE usar OpenAI se a key estiver disponível
    ...
    if (useOpenAI) {
      try {
        console.log('🤖 Usando OpenAI GPT-4 para resposta terapêutica contextual');
        console.log(`📝 Contexto: ${conversationContext.messages.length} mensagens na conversa`);
        ...
        console.log(`✅ Resposta OpenAI gerada com contexto | Emoções: ${emotions.join(', ')} | Risco: ${riskLevel}`);
      ...
      console.log('⚠️ OPENAI_API_KEY não encontrada - usando sistema programático');
      console.log('Configure a variável de ambiente OPENAI_API_KEY para respostas mais inteligentes');
