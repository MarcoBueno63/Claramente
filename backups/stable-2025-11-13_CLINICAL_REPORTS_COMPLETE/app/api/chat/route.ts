  ...
  try {
    ...
    const conversationContext: ConversationContext = {
      messages: [
        ...session.messages.map(m => ({
          role: m.content.includes('Sou a ClaraMente') ? 'assistant' as const : 'user' as const,
          content: m.content,
    ...
    if (useOpenAI) {
      try {
        ...
        response = generateResponse(userMessage, emotions, timeElapsed, session.messages);
      }
    } else {
