...
export default function AvatarWithLipSync({
  ...
  // Renderizar avatar em canvas (apenas a imagem, sem desenho de boca)
  useEffect(() => {
    ...
    img.onload = () => {
      // Desenhar apenas a imagem, sem modificações
    ...
    <div className={`relative ${className}`}>
      ...
      {mode === 'canvas' && (
        <div className="relative w-full h-full">
          ...
          {isSpeaking && (
            <div className="absolute bottom-4 left-1/2 transform -translate-x-1/2 flex items-end gap-1 bg-white/90 backdrop-blur-sm px-3 py-2 rounded-full shadow-lg">
              <div 
                className="w-1 bg-gradient-to-t from-pink-500 to-pink-300 rounded-full transition-all duration-100"
                style={{ 
                  height: `${12 + mouthOpenness * 20}px`,
                  animation: 'pulse 0.6s ease-in-out infinite'
                }}
              />
              <div 
                className="w-1 bg-gradient-to-t from-purple-500 to-purple-300 rounded-full transition-all duration-100"
                style={{ 
                  height: `${16 + mouthOpenness * 24}px`,
                  animationDelay: '0.1s',
                  animation: 'pulse 0.6s ease-in-out infinite'
                }}
              />
              <div 
                className="w-1 bg-gradient-to-t from-pink-600 to-pink-400 rounded-full transition-all duration-100"
                style={{ 
                  height: `${20 + mouthOpenness * 28}px`,
                  animationDelay: '0.2s',
                  animation: 'pulse 0.6s ease-in-out infinite'
                }}
              />
              ...
              />
              <span className="ml-2 text-xs font-medium text-gray-700">Falando...</span>
            </div>
          )}
        </div>
      )}
