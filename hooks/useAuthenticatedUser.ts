...
"use client";
// import { useSession } from "next-auth/react"; // Removido para otimização
...
export function useAuthenticatedUser(): AuthenticatedUserState {
  ...
  useEffect(() => {
    const initializeUser = async () => {
      ...
      try {
        if (session) {
          // Usuário autenticado via sistema simplificado
  ...
  const endCurrentSession = (moodAfter?: number): TherapySession | null => {
    ...
    try {
      ...
      if (finalizedSession && user) {
        ...
        const updatedUser = authService.useFreeSession(user);
        setUser(updatedUser);
